      ...
      detector_(geometry.clone())
{
    ...
    for (auto sector : sectors) {
        if (sector->GetParticleDef() != particle_def_) {
            ...
            if (sector->GetUtility().only_stochastic)
                sectors_.push_back(new SectorStochastic(*sector));
            else
                sectors_.push_back(new SectorMixed(*sector));
        }
    }
    ...
    , detector_(geometry.clone())
{
    for (auto def : sector_defs) {
        if(def.utility_def.only_stochastic)
            sectors_.push_back(new SectorStochastic(particle_def, def));
        else
            sectors_.push_back(new SectorMixed(particle_def, def));
    }
    ...
    , detector_(geometry.clone())
{
    for (auto def : sector_defs) {
        if(def.utility_def.only_stochastic)
            sectors_.push_back(new SectorStochastic(particle_def, def, interpolation_def));
        else
            sectors_.push_back(new SectorMixed(particle_def, def, interpolation_def));
    }
    ...
    , detector_(propagator.detector_->clone())
{
    for (unsigned int i = 0; i < propagator.sectors_.size(); ++i) {
        if((*propagator.sectors_[i]).GetUtility().only_stochastic)
            sectors_[i] = new SectorStochastic(particle_def_, *propagator.sectors_[i]);
        else
            sectors_[i] = new SectorMixed(particle_def_, *propagator.sectors_[i]);
    ...
    , detector_(NULL)
{
    ...
    for (size_t idx = 0; idx < json_config["sectors"].size(); idx++) {
        ...
        if(sec_def_global.utility_def.only_stochastic){
            if (do_interpolation) {
                sectors_.push_back(
                    new SectorStochastic(particle_def_, sec_def_infront, interpolation_def));
                sectors_.push_back(
                    new SectorStochastic(particle_def_, sec_def_inside, interpolation_def));
                sectors_.push_back(
                    new SectorStochastic(particle_def_, sec_def_behind, interpolation_def));
            } else {
                sectors_.push_back(new SectorStochastic(particle_def_, sec_def_infront));
                sectors_.push_back(new SectorStochastic(particle_def_, sec_def_inside));
                sectors_.push_back(new SectorStochastic(particle_def_, sec_def_behind));
            }
        } else {
            if (do_interpolation) {
                sectors_.push_back(
                    new SectorMixed(particle_def_, sec_def_infront, interpolation_def));
                sectors_.push_back(
                    new SectorMixed(particle_def_, sec_def_inside, interpolation_def));
                sectors_.push_back(
                    new SectorMixed(particle_def_, sec_def_behind, interpolation_def));
            } else {
                sectors_.push_back(new SectorMixed(particle_def_, sec_def_infront));
                sectors_.push_back(new SectorMixed(particle_def_, sec_def_inside));
                sectors_.push_back(new SectorMixed(particle_def_, sec_def_behind));
            }
        }
    ...
    DynamicData initial_condition, double max_distance, double minimal_energy)
{
    ...
    while (1) {
        ...
        bool fatal = p_condition->GetTypeId() != static_cast<int>(InteractionType::Particle) &&
                     p_condition->GetTypeId() != static_cast<int>(InteractionType::ContinuousEnergyLoss) &&
                     (p_condition->GetTypeId() == static_cast<int>(InteractionType::Decay) ||
                     current_sector_->GetUtility().GetCrosssection(p_condition->GetTypeId())->GetParametrization().isFatal());
    ...
    const std::string& json_object_str)
{
    ...
    if (json_global.find("weak_multiplier_nc") != json_global.end()) {
        if (json_global["weak_multiplier_nc"].is_number()) {
            sec_def_global.utility_def.weaknc_def.multiplier
                = json_global["weak_multiplier_nc"].get<double>();
        }
        else {
            log_fatal("Invalid input for option 'weak_multiplier_nc'. Expected a "
                      "number.");
        }
    } else {
        log_debug("The 'weak_multiplier_nc' option is not set. Use default (%f)",
            sec_def_global.utility_def.weak_def.multiplier);
    }
    ...
    if (json_global.find("weaknc") != json_global.end()) {
        if (json_global["weaknc"].is_string()) {
            ...
            sec_def_global.utility_def.weaknc_def.parametrization
                = WeakInteractionFactory_NC::Get().GetEnumFromString(weaknc);
        } else {
            log_fatal("Invalid input for option 'weaknc'. Expected a string.");
        }
    } else {
        log_debug("The 'weaknc' option is not set. Use default %s",
            WeakInteractionFactory_NC::Get()
                .GetStringFromEnum(
                    sec_def_global.utility_def.weaknc_def.parametrization)
                .c_str());
    }
