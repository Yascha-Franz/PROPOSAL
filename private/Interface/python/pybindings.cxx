...
public:
    ...
    double CalculateTime(const DynamicData& p_condition,
        const double final_energy, const double displacement) override {
        PYBIND11_OVERLOAD_PURE(
            double,                                      /* Return type */
            Sector,                                      /* Parent class */
            CalculateTime,                               /* Name of function in C++ (must match Python name) */
            p_condition, final_energy, displacement      /* Argument(s) */
        );
    }

    int ChooseLoss(const std::array<double, 4>& Losses) override {
        PYBIND11_OVERLOAD_PURE(
            int,                     /* Return type */
            Sector,                  /* Parent class */
            ChooseLoss,              /* Name of function in C++ (must match Python name) */
            Losses                   /* Argument(s) */
        );
    }
    
    double CalculateDecay(const double initial_energy, const double rnd) override {
        PYBIND11_OVERLOAD_PURE(
            double,                  /* Return type */
            Sector,                  /* Parent class */
            CalculateDecay,          /* Name of function in C++ (must match Python name) */
            initial_energy, rnd      /* Argument(s) */
        );   
    }
    
    double CalculateInteraction(const double initial_energy, const double rnd) override {
        PYBIND11_OVERLOAD_PURE(
            double,                  /* Return type */
            Sector,                  /* Parent class */
            CalculateInteraction,    /* Name of function in C++ (must match Python name) */
            initial_energy, rnd      /* Argument(s) */
        );   
    }
    
    double CalculateDistance(const double initial_energy, const double distance) override {
        PYBIND11_OVERLOAD_PURE(
            double,                  /* Return type */
            Sector,                  /* Parent class */
            CalculateDistance,       /* Name of function in C++ (must match Python name) */
            initial_energy, distance /* Argument(s) */
        );   
    }
    
    double CalculateMinimalEnergy(const double initial_energy, const double rnd) override {
        PYBIND11_OVERLOAD_PURE(
            double,                  /* Return type */
            Sector,                  /* Parent class */
            CalculateMinimalEnergy,  /* Name of function in C++ (must match Python name) */
            initial_energy, rnd      /* Argument(s) */
        );   
    }

    Secondaries Propagate(const DynamicData& particle_condition,
        double max_distance=1e20, double minimal_energy=0.) override {
        PYBIND11_OVERLOAD_PURE(
            Secondaries,                                         /* Return type */
            Sector,                                              /* Parent class */
            Propagate,                                           /* Name of function in C++ (must match Python name) */
            particle_condition, max_distance, minimal_energy     /* Argument(s) */
        );
    }
};
...
PYBIND11_MODULE(pyPROPOSAL, m)
{
    ...
    py::class_<Sector, pySector, std::shared_ptr<Sector>>(m, "Sector", R"pbdoc(
    ...
    py::class_<SectorMixed, Sector, std::shared_ptr<SectorMixed>>(m, "SectorMixed", R"pbdoc(
            General Sector for most propagations.
            Needs at least one Interaction with continuos losses to work.
            A sector is characterized by its homogeneous attitudes.
            Within a sector there are no boundaries to consider.
		)pbdoc")
        .def(py::init<ParticleDef&, const SectorMixed::Definition&>(),
            py::arg("particle_def"), py::arg("sector_definition"))
        .def(py::init<ParticleDef&, const SectorMixed::Definition&,
                 const InterpolationDef&>(),
            py::arg("particle_def"), py::arg("sector_definition"),
            py::arg("interpolation_def"))
        .def("calculate_decay", &SectorMixed::CalculateDecay, py::arg("initial_energy"),
            py::arg("rnd"))
        .def("calculate_interaction", &SectorMixed::CalculateInteraction,
            py::arg("initial_energy"), py::arg("rnd"))
        .def("calculate_minimal_energy", &SectorMixed::CalculateMinimalEnergy,
            py::arg("initial_energy"), py::arg("cut"))
        .def("calculate_distance", &SectorMixed::CalculateDistance,
            py::arg("initial_energy"), py::arg("distance"))
        .def("make_stochastic_loss", &SectorMixed::MakeStochasticLoss,
            py::arg("minimal_energy"))
        .def("propagate", &SectorMixed::Propagate,
            py::arg("particle_condition"), py::arg("max_distance"), py::arg("min_energy"));

    py::class_<SectorStochastic, Sector, std::shared_ptr<SectorStochastic>>(m, "SectorStochastic", R"pbdoc(
            Sepecialised Sector for purely stochastic losses.
            All continuos parametrisations are disregarded.
            A sector is characterized by its homogeneous attitudes.
            Within a sector there are no boundaries to consider.
		)pbdoc")
        .def(py::init<ParticleDef&, const SectorStochastic::Definition&>(),
            py::arg("particle_def"), py::arg("sector_definition"))
        .def(py::init<ParticleDef&, const SectorStochastic::Definition&,
                 const InterpolationDef&>(),
            py::arg("particle_def"), py::arg("sector_definition"),
            py::arg("interpolation_def"))
        .def("calculate_decay", &SectorStochastic::CalculateDecay,
            py::arg("initial_energy"), py::arg("rnd"))
        .def("calculate_interaction", &SectorStochastic::CalculateInteraction,
            py::arg("initial_energy"), py::arg("rnd"))
        .def("calculate_minimal_energy", &SectorStochastic::CalculateMinimalEnergy,
            py::arg("initial_energy"), py::arg("cut"))
        .def("calculate_distance", &SectorStochastic::CalculateDistance,
            py::arg("initial_energy"), py::arg("distance"))
        .def("propagate", &SectorStochastic::Propagate,
            py::arg("particle_condition"), py::arg("max_distance"), py::arg("min_energy"));
