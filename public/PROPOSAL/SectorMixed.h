 ...
 */
class SectorMixed : public Sector{
public:
    // Sector(Particle&);
    SectorMixed(const ParticleDef&, const Sector::Definition&);
    SectorMixed(const ParticleDef&, const Sector::Definition&, const InterpolationDef&);
    SectorMixed(const ParticleDef&, const Sector&);
    ...
    int ChooseLoss(const std::array<double, 4>& LossEnergies);


    std::shared_ptr<DynamicData> DoContinuous(
        const DynamicData&, double, double);
    void MakeStochasticStep();
